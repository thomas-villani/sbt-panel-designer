...
| **FAS feedback round 3** (2026-08-27) — open channels, manual metals, empty channels, sort, new panel, spill table, thresholds, viability/barcoding choices, structural sets, off-list metals, wording, contrast | 🟢 shipped | Sidebar metal pill is a dropdown (open channels / swap / custom conjugation of your own); `OpenChannels` chips on Balance + click-to-keep-empty on the mass strip; sort by added / name / metal; **New** button clears the panel *and* the share hash (the leftover `#...` read as a file name); `SpillTable` (marker · metal · spill in · spill out · status, hover for detail) is the Balance default with the triage cards kept for must-fix; **SBT kits arrive pinned to their kit metals** (was: re-optimised, and kit-only vials like CD66b-172Yb could not even be placed) and count as one SKU on Order ("in kit"); thresholds recalibrated against every SBT kit run on its metals (warn ≥ 1 ×, must-fix ≥ 2 × tolerance; MDIPA peaks at 1.75) and spill between two markers of one kit is filed "validated in kit"; viability = natural Pt / 195Pt / 198Pt / Rh103, barcoding = Cell-ID 20-Plex Pd; `structural` category (5 IMC sets + the TME one); `instruments.yaml: advanced` = opt-in metals (Cd on IMC, assumed sensitivity 0.3) flagged on Balance/Order; "Scaffolding" → "Cell ID & controls", "module" → "marker set"; slate-500 → 600 and stronger selected tiles. Open: named barcoding schemes beyond Pd once SBT lists them; a Cd sensitivity for Hyperion; kit `role: optional` markers still hidden on cards. |
...
7. **2026-08-27 (FAS round 3):** which off-list metals deserve an opt-in besides Cd on IMC (La? 194Pt / 197Au?) and a real Hyperion sensitivity for Cd (we assume 0.3); the barcoding schemes to offer besides Cell-ID 20-Plex Pd; confirmation that the recalibrated spill thresholds (warn ≥ 1 ×, must-fix ≥ 2 × tolerance, kits validated as sets) match what FAS would flag on the two-kit panel they showed us.
